...
# Singleton Keepa instance — her çağrıda yeni nesne oluşturma
_keepa_api = None

def _get_keepa_api():
    """Keepa API singleton — yalnızca bir kez init edilir"""
    global _keepa_api
    if _keepa_api is None and KEEPA_AVAILABLE and KEEPA_API_KEY:
        _keepa_api = keepa.Keepa(KEEPA_API_KEY)
    return _keepa_api


def _has_enough_tokens(min_tokens: int = TOKEN_GUARD) -> bool:
    """Token guard — yetersiz token varsa API'ye gitme"""
    try:
        api = _get_keepa_api()
        if api is None:
            return False
        return api.tokens_left >= min_tokens
    except Exception:
        return False
...
def get_token_status() -> dict:
    """Keepa token durumunu kontrol et — singleton kullan, yeni çağrı yapma"""
    if not KEEPA_AVAILABLE or not KEEPA_API_KEY:
    ...
    try:
        ...
        return {
            ...
            "guard_threshold": TOKEN_GUARD,
            "safe_to_query": api.tokens_left >= TOKEN_GUARD,
        }
    except Exception as e:
