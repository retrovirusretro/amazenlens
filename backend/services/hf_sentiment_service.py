...
async def get_keyword_embeddings(keywords: list) -> list:
    """sentence-transformers/all-MiniLM ile 384-dim embedding vektörleri"""
    model = "sentence-transformers/all-MiniLM-L6-v2"
    if not HF_API_TOKEN or not keywords:
        return []
    try:
        async with httpx.AsyncClient(timeout=30) as client:
...
def cluster_keywords_by_embedding(keywords: list, embeddings: list, threshold: float = 0.7) -> list:
    """Cosine similarity ile keyword'leri semantik gruplara ayır"""
    import numpy as np

    if not keywords or not embeddings or len(keywords) != len(embeddings):
    ...
    for i, kw in enumerate(keywords):
        ...
        for j in range(i + 1, len(keywords)):
            if j in assigned:
                continue
            sim = float(np.dot(arrs[i], arrs[j]) / (np.linalg.norm(arrs[i]) * np.linalg.norm(arrs[j]) + 1e-9))
            if sim >= threshold:
...
async def classify_product_category(title: str) -> dict:
    """facebook/bart-large-mnli ile zero-shot kategori tespiti"""
    model = ZERO_SHOT_MODEL
    categories = [
        "Sports & Outdoors", "Home & Kitchen", "Electronics",
        "Health & Personal Care", "Toys & Games", "Beauty",
        "Clothing & Apparel", "Pet Supplies", "Office Products",
        "Baby Products", "Automotive", "Garden & Outdoor",
    ]
    if not HF_API_TOKEN or not title:
        return {"category": "Unknown", "confidence": 0.0, "mock": True}
    try:
        async with httpx.AsyncClient(timeout=30) as client:
            resp = await client.post(
                f"{HF_BASE_URL}/{model}",
                headers=HEADERS,
                json={"inputs": title, "parameters": {"candidate_labels": categories}}
            )
            if resp.status_code == 200:
                data = resp.json()
                if "labels" in data:
                    return {
                        ...
                        "all": dict(zip(data["labels"][:5], [round(s, 3) for s in data["scores"][:5]]))
                    }
    except Exception as e:
