...
async def search_suppliers(keyword: str, page: int = 1):
    """Alibaba tedarikçi arama — önce Playwright Worker, yoksa mock"""
    from services.playwright_client import scrape_alibaba, WORKER_AVAILABLE

    if WORKER_AVAILABLE:
        try:
            result = await scrape_alibaba(keyword, max_results=20)
            if result.get("count", 0) > 0:
                # Playwright sonuçlarını bizim formata dönüştür
                suppliers = []
                for p in result.get("results", []):
                    price = p.get("price", 0)
                    price_max = p.get("price_max") or price
                    suppliers.append({
                        "name": p.get("supplier") or "Alibaba Supplier",
                        "product": p.get("title", ""),
                        "price": f"${price:.2f}" + (f" – ${price_max:.2f}" if price_max != price else ""),
                        "price_usd": price,
                        "moq": str(p.get("moq", "1")),
                        "url": p.get("url", ""),
                        "image": p.get("image", ""),
                        "country": p.get("country", "CN"),
                        "source": "alibaba_live",
                    })
                return {
                    "keyword": keyword,
            ...
            print(f"Alibaba playwright error: {e}")

    return get_mock_suppliers(keyword)


async def search_dhgate(keyword: str):
    """DHgate tedarikçi arama — Playwright Worker veya mock"""
    from services.playwright_client import scrape_dhgate as pw_dhgate, WORKER_AVAILABLE

    if WORKER_AVAILABLE:
        try:
            result = await pw_dhgate(keyword, max_results=20)
            if result.get("count", 0) > 0:
                return result
        except Exception as e:
            print(f"DHgate playwright error: {e}")
