...
def _to_int(val, default=0):
    try:
        if isinstance(val, str):
            val = val.replace(',', '').strip().split('.')[0]
        return int(float(val)) if val else default
    except (ValueError, TypeError):
        return default

def calculate_niche_score(product: dict, keepa_data: dict = None) -> dict:
    dimensions_data = product.get("dimensions", {}) or {}
    weight = _to_float(dimensions_data.get("weight", 0))
    title = str(product.get("title", "") or "").lower()
    price = _to_float(product.get("price", 0))
    reviews = _to_int(product.get("reviews_count", product.get("reviews", 9999))) or 9999
    bsr = _to_int(product.get("bestseller_rank", product.get("bsr", 999999))) or 999999
    rating = _to_float(product.get("rating", 0))
