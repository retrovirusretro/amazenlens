...
async def add_points(data: PointRequest):
    ...
    points = POINT_LIMITS.get(data.action, 0)
    if points == 0:
        raise HTTPException(status_code=400, detail="Gecersiz action")
    ...
    try:
        ...
        supabase.table("profiles").update({
            "lens_points": current + points
        }).eq("id", data.user_id).execute()

        return {"success": True, "total_points": current + points}
    except Exception as e:
...
async def get_feedback(secret: str = ""):
    ...
    if not admin_secret:
        raise HTTPException(status_code=503, detail="Admin secret yapilandirilmamis")
    ...
    try:
        ...
        print(f"Feedback list hatasi: {e}")
        raise HTTPException(status_code=500, detail="Bir hata olustu")
