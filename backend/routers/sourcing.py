...
@router.get("/trendyol")
async def trendyol_search(
    keyword: str = Query(..., description="Arama kelimesi"),
    max_results: int = Query(10, description="Maksimum sonuç")
):
    """Trendyol'da gerçek fiyat çek — Playwright Worker (ayrı Railway servisi)"""
    from services.playwright_client import scrape_trendyol
    return await scrape_trendyol(keyword, max_results)


@router.get("/worker-health")
async def playwright_worker_health():
    """Playwright worker servis durumu"""
    from services.playwright_client import worker_health
    return await worker_health()
...
from services.mediamarkt import search_mediamarkt, compare_amazon_mediamarkt

@router.get("/mediamarkt")
async def mediamarkt_search(
    keyword: str = Query(...),
    market: str = Query("DE", description="DE/TR/NL/AT/ES/IT/BE/CH")
):
    """MediaMarkt'ta ürün ara — ScraperAPI ile gerçek veri"""
    return await search_mediamarkt(keyword, market)

@router.get("/mediamarkt-compare")
async def mediamarkt_compare(
    keyword: str = Query(...),
    amazon_price: float = Query(...),
    market: str = Query("DE")
):
    """Amazon vs MediaMarkt fiyat karşılaştırması + arbitraj"""
    return await compare_amazon_mediamarkt(keyword, amazon_price, market)
