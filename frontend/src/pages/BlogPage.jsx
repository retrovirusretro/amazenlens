      ...
      <div style={{display: 'flex', gap: '8px', marginBottom: '32px', flexWrap: 'wrap', justifyContent: 'center'}}>
        <button
          ...
          style={{
            ...
            background: !selectedCategory ? '#3b82f6' : '#f1f5f9',
        ...
        <div style={{display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '24px'}}>
          {posts.map(post => (
            <div key={post.id}
              ...
              {/* Cover */}
              <div style={{
                height: '160px', background: 'linear-gradient(135deg, #3b82f6, #8b5cf6)',
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                fontSize: '48px'
              }}>
                {post.category === 'Başlangıç' ? '🚀' :
                 post.category === 'Ürün Araştırması' ? '🔍' :
                 post.category === 'Arbitraj' ? '💱' :
                 post.category === 'Tedarik' ? '🏭' :
                 post.category === 'Finansal' ? '💰' : '📝'}
              </div>
