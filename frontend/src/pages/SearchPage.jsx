...
const SortHeader = ({ label, sortKey, currentKey, currentDir, onClick, align = 'right' }) => (
  <div onClick={onClick} style={{
    textAlign: align, cursor: sortKey ? 'pointer' : 'default',
    color: currentKey === sortKey ? '#0071e3' : '#aeaeb2',
    display: 'flex', alignItems: 'center',
    justifyContent: align === 'right' ? 'flex-end' : 'flex-start', gap: '3px',
    userSelect: 'none'
  }}>
    {label}
    {sortKey && (
...
function SearchPage() {
  ...
  return (
    <div style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      {/* Topbar Kartı */}
      <div style={{
        background: 'white', borderRadius: '12px', border: '0.5px solid #e5e5ea',
        padding: '14px 16px', marginBottom: '12px'
      }}>
        {/* Başlık + Pazar */}
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '12px' }}>
          <div style={{ fontSize: '18px', fontWeight: '600', color: '#1d1d1f', letterSpacing: '-0.3px' }}>
            Ürün Araştırması
          </div>
          <div style={{ position: 'relative' }}>
            <div onClick={() => setShowMarketDD(!showMarketDD)} style={{
              display: 'flex', alignItems: 'center', gap: '6px',
              background: '#f5f5f7', border: '0.5px solid #d2d2d7',
              borderRadius: '8px', padding: '7px 12px', cursor: 'pointer', fontSize: '13px', color: '#1d1d1f'
            }}>
              <span>{currentMarket.flag}</span>
              <span>{currentMarket.label}</span>
              <svg width="11" height="11" fill="none" stroke="#aeaeb2" strokeWidth="1.5" viewBox="0 0 24 24">
            ...
            {showMarketDD && (
              <div style={{
                ...
                border: '0.5px solid #d2d2d7', borderRadius: '10px', padding: '6px',
                zIndex: 100, minWidth: '160px', boxShadow: '0 4px 20px rgba(0,0,0,0.08)'
              }}>
                {MARKETS.map(m => (
                  <div key={m.value} onClick={() => { setSelectedMarket(m.value); setShowMarketDD(false) }}
                    style={{
                      ...
                      background: selectedMarket === m.value ? '#f0f0f5' : 'transparent',
                      fontWeight: selectedMarket === m.value ? '500' : '400', color: '#1d1d1f'
                    }}>
        ...
        <div style={{ display: 'flex', gap: '8px', marginBottom: '12px' }}>
          ...
          <button onClick={() => setShowFilterPanel(!showFilterPanel)} style={{
            background: showFilterPanel ? '#1d1d1f' : 'white',
            color: showFilterPanel ? 'white' : '#1d1d1f',
            border: '0.5px solid #d2d2d7', padding: '9px 14px',
            borderRadius: '8px', fontSize: '13px', cursor: 'pointer',
            fontFamily: 'inherit', display: 'flex', alignItems: 'center', gap: '6px', whiteSpace: 'nowrap'
          }}>
        ...
        <div style={{ display: 'flex', gap: '6px', overflowX: 'auto', paddingBottom: '2px' }}>
          {CATEGORIES.map(cat => (
            <div key={cat.value} onClick={() => setSelectedCat(cat.value)} style={{
              fontSize: '12px', padding: '5px 12px', borderRadius: '20px',
              border: `0.5px solid ${selectedCat === cat.value ? '#1d1d1f' : '#d2d2d7'}`,
              background: selectedCat === cat.value ? '#1d1d1f' : 'white',
              color: selectedCat === cat.value ? 'white' : '#3c3c43',
      ...
      {showFilterPanel && (
        <div style={{
          background: 'white', borderRadius: '10px', border: '0.5px solid #e5e5ea',
            ...
            <div key={f.key}>
              ...
              <input type="number" placeholder={f.placeholder}
                ...
                style={{
                  ...
                  border: '0.5px solid #d2d2d7', fontSize: '13px',
                  fontFamily: 'inherit', outline: 'none', color: '#1d1d1f', boxSizing: 'border-box'
                }}
              />
            </div>
          ))}
          <div style={{ gridColumn: '1/-1', display: 'flex', justifyContent: 'flex-end', gap: '8px', marginTop: '4px' }}>
            <button onClick={() => setFilters({ priceMin: '', priceMax: '', bsrMax: '', reviewsMax: '', ratingMin: '' })}
      ...
      {filtered.length > 0 && (
        <div style={{ fontSize: '13px', color: '#8e8e93', marginBottom: '10px', padding: '0 4px' }}>
      ...
      {loading && (
        <div style={{ textAlign: 'center', padding: '60px', color: '#8e8e93' }}>
          <div style={{
            width: '28px', height: '28px', border: '2px solid #f0f0f5',
            borderTop: '2px solid #0071e3', borderRadius: '50%',
      ...
      {!loading && filtered.length > 0 && (
        <>
          ...
          <div style={{
            display: 'grid',
            gridTemplateColumns: '46px 1fr 62px 70px 78px 78px 68px 54px 36px',
            gap: '10px', padding: '0 16px 8px',
            fontSize: '10px', letterSpacing: '0.4px'
          }}>
            <div></div>
            <div style={{ color: '#aeaeb2', textTransform: 'uppercase' }}>Ürün</div>
            <SortHeader label="Fiyat" sortKey="price" currentKey={sortKey} currentDir={sortDir} onClick={() => handleSort('price')} />
            <div style={{ color: '#aeaeb2', textTransform: 'uppercase', textAlign: 'center' }}>Trend</div>
            ...
            {filtered.map((product) => {
              ...
              return (
                <div key={product.asin}
                  ...
                  style={{
                    background: 'white', borderRadius: '10px', border: '0.5px solid #e5e5ea',
                    padding: '11px 16px', cursor: 'pointer', transition: 'border-color 0.15s',
                    display: 'grid',
                    gridTemplateColumns: '46px 1fr 62px 70px 78px 78px 68px 54px 36px',
                ...
                >
                  ...
                  <div style={{ minWidth: 0 }}>
                    <div style={{ fontSize: '13px', fontWeight: '500', color: '#1d1d1f', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', marginBottom: '4px' }}>
                      {product.title}
                    </div>
                    <div style={{ display: 'flex', gap: '5px', alignItems: 'center' }}>
                      <span style={{ fontSize: '10px', color: '#aeaeb2', fontFamily: 'monospace' }}>{product.asin}</span>
                      {product.in_stock === false && (
                        <span style={{ fontSize: '10px', padding: '1px 6px', borderRadius: '3px', background: '#fff1f0', color: '#c00', fontWeight: '500' }}>Stokta Yok</span>
                      )}
                    </div>
                  ...
                  {/* Fiyat */}
                  <div style={{ textAlign: 'right', fontSize: '13px', fontWeight: '500', color: '#1d1d1f' }}>
                  ...
                  <div style={{ textAlign: 'right' }}>
                    <div style={{
                      ...
                      background: SCORE_BG(score), fontSize: '11px', fontWeight: '600', color: SCORE_TEXT(score)
                    }}>
                      {score || '-'}
                    </div>
                  </div>
