...
export default function ProductPage() {
  ...
  const fetchVision = async () => {
    if (!product?.images?.[0] || visionLoading) return
    setVisionLoading(true)
    try {
      const res = await axios.post(`${API}/api/ai/analyze-image`, { image_url: product.images[0] })
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          {!visionData ? (
            <div style={{ background: 'white', borderRadius: '12px', border: '0.5px solid #e5e5ea', padding: '40px', textAlign: 'center' }}>
              ...
              <div style={{ fontSize: '14px', fontWeight: '600', color: '#1d1d1f', marginBottom: '6px' }}>AI Görsel Analizi</div>
              <div style={{ fontSize: '12px', color: '#8e8e93', marginBottom: '4px' }}>Ürün görselini analiz et — renk, materyal, ambalaj kalitesi, foto önerileri</div>
              <div style={{ fontSize: '11px', color: '#8e8e93', marginBottom: '20px' }}>Claude Haiku Vision · ~3 saniye</div>
              {product?.images?.[0] && (
                <img src={product.images[0]} alt="" style={{ width: 80, height: 80, objectFit: 'contain', borderRadius: 8, marginBottom: 16, border: '0.5px solid #e5e5ea' }} />
              )}
              <div>
                <button onClick={fetchVision} disabled={visionLoading || !product?.images?.[0]}
                  style={{ padding: '10px 28px', background: 'linear-gradient(135deg,#0071e3,#34aadc)', color: 'white', border: 'none', borderRadius: '8px', fontSize: '13px', fontWeight: '600', cursor: 'pointer', fontFamily: 'inherit', opacity: !product?.images?.[0] ? 0.5 : 1 }}>
                  {visionLoading ? '⏳ Analiz Ediliyor...' : '🔍 Görseli Analiz Et'}
                </button>
              </div>
              {!product?.images?.[0] && <div style={{ fontSize: '11px', color: '#ff3b30', marginTop: 8 }}>Ürün görseli bulunamadı</div>}
            </div>
          ) : (
            <>
              {/* Özet skorlar */}
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '8px' }}>
                {[
                  { label: 'Genel Skor', value: visionData.overall_score, icon: '⭐' },
                  { label: 'Foto Kalitesi', value: visionData.photo_quality_score, icon: '📷' },
                  { label: 'Ambalaj', value: visionData.packaging_score, icon: '📦' },
                ].map(item => (
                  <div key={item.label} style={{ background: 'white', borderRadius: '12px', border: '0.5px solid #e5e5ea', padding: '16px', textAlign: 'center' }}>
                    <div style={{ fontSize: '20px', marginBottom: '4px' }}>{item.icon}</div>
                    <div style={{ fontSize: '22px', fontWeight: '700', color: item.value >= 70 ? '#34c759' : item.value >= 50 ? '#ff9f0a' : '#ff3b30' }}>{item.value}</div>
                    <div style={{ fontSize: '11px', color: '#8e8e93' }}>{item.label}</div>
                  </div>
                ))}
              </div>
              ...
              {/* Detaylar */}
              <div style={{ background: 'white', borderRadius: '12px', border: '0.5px solid #e5e5ea', padding: '16px 20px' }}>
                <div style={{ fontSize: '13px', fontWeight: '600', color: '#1d1d1f', marginBottom: '12px' }}>Ürün Detayları</div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  {[
                    { label: 'Renk(ler)', value: visionData.colors?.join(', ') },
                    { label: 'Materyal', value: visionData.material },
                    { label: 'Boyut Tahmini', value: visionData.size_estimate },
                  ].map(row => (
                    <div key={row.label} style={{ display: 'flex', gap: '8px', alignItems: 'flex-start' }}>
                      <span style={{ fontSize: '12px', color: '#8e8e93', minWidth: '110px', flexShrink: 0 }}>{row.label}</span>
                      <span style={{ fontSize: '12px', color: '#1d1d1f', fontWeight: '500' }}>{row.value || '—'}</span>
                    </div>
                  ))}
                </div>
              </div>

              {/* Foto & Ambalaj Notları */}
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
                <div style={{ background: 'white', borderRadius: '12px', border: '0.5px solid #e5e5ea', padding: '14px' }}>
                  <div style={{ fontSize: '12px', fontWeight: '600', color: '#1d1d1f', marginBottom: '6px' }}>📷 Foto Notu</div>
                  <div style={{ fontSize: '12px', color: '#3c3c43' }}>{visionData.photo_notes || '—'}</div>
                </div>
                <div style={{ background: 'white', borderRadius: '12px', border: '0.5px solid #e5e5ea', padding: '14px' }}>
                  <div style={{ fontSize: '12px', fontWeight: '600', color: '#1d1d1f', marginBottom: '6px' }}>📦 Ambalaj Notu</div>
                  <div style={{ fontSize: '12px', color: '#3c3c43' }}>{visionData.packaging_notes || '—'}</div>
                </div>
              </div>

              {/* Etiketler */}
              {visionData.tags?.length > 0 && (
                <div style={{ background: 'white', borderRadius: '12px', border: '0.5px solid #e5e5ea', padding: '14px 16px' }}>
                  <div style={{ fontSize: '12px', fontWeight: '600', color: '#1d1d1f', marginBottom: '8px' }}>Etiketler</div>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
                    {visionData.tags.map(tag => (
                      <span key={tag} style={{ fontSize: '11px', background: '#f2f2f7', color: '#3c3c43', padding: '4px 10px', borderRadius: '20px' }}>{tag}</span>
                    ))}
                  </div>
                </div>
              )}
              ...
              {/* Listing İpuçları */}
              {visionData.listing_tips?.length > 0 && (
                <div style={{ background: 'white', borderRadius: '12px', border: '0.5px solid #e5e5ea', padding: '14px 16px' }}>
                  <div style={{ fontSize: '12px', fontWeight: '600', color: '#1d1d1f', marginBottom: '8px' }}>💡 Listing Önerileri</div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                    {visionData.listing_tips.map((tip, i) => (
                      <div key={i} style={{ fontSize: '12px', color: '#3c3c43', display: 'flex', gap: '8px', alignItems: 'flex-start' }}>
                        <span style={{ color: '#0071e3', flexShrink: 0 }}>→</span>{tip}
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <div style={{ textAlign: 'center' }}>
                <button onClick={() => setVisionData(null)} style={{ fontSize: '12px', color: '#8e8e93', background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'inherit' }}>
                  Yeniden Analiz Et
                </button>
              </div>
            </>
