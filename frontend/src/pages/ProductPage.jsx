...
export default function ProductPage() {
  ...
  const fetchReviews = async () => {
    ...
    try {
      ...
      const res = await axios.get(`${API}/api/reviews/analyze/${asin}?title=${title}`)
      setReviews(res.data)
    } catch (err) {
      console.error(err)
    } finally {
  ...
  const tabs = [
    ...
    { key: 'lovehate', label: '💚❤️ Yorum Analizi' },
        ...
        <div>
          {reviewsLoading ? (
            <div style={{ background: 'white', borderRadius: '12px', border: '0.5px solid #e5e5ea', padding: '60px', textAlign: 'center' }}>
              ...
              <div style={{ fontSize: '11px', color: '#aeaeb2', marginTop: '6px' }}>Bu birkaç saniye sürebilir</div>
            </div>
          ) : reviews ? (
            <div>
              {/* Sentiment Bar */}
              <div style={{ background: 'white', borderRadius: '12px', border: '0.5px solid #e5e5ea', padding: '16px 20px', marginBottom: '12px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <div style={{ fontSize: '22px', fontWeight: '600', color: reviews.sentiment_score >= 70 ? '#34c759' : reviews.sentiment_score >= 50 ? '#ff9f0a' : '#ff3b30' }}>
                      %{reviews.sentiment_score}
                    </div>
                    {reviews.mock && (
                      <span style={{ fontSize: '10px', padding: '2px 8px', borderRadius: '4px', background: '#f5f5f7', color: '#8e8e93' }}>
                        Demo Veri
                      </span>
                    )}
                  </div>
                ...
                <div style={{ height: '6px', background: '#f0f0f5', borderRadius: '3px', marginBottom: '8px' }}>
                  <div style={{ height: '100%', borderRadius: '3px', background: reviews.sentiment_score >= 70 ? '#34c759' : reviews.sentiment_score >= 50 ? '#ff9f0a' : '#ff3b30', width: `${reviews.sentiment_score}%` }}></div>
                </div>
                <div style={{ fontSize: '12px', color: '#8e8e93' }}>
                  {reviews.total_reviews_analyzed} yorum analiz edildi · {reviews.summary}
                </div>
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                ...
                <div style={{ background: 'white', borderRadius: '12px', border: '0.5px solid #e5e5ea', overflow: 'hidden' }}>
                  <div style={{ padding: '14px 16px 10px', background: '#e8f9ee', borderBottom: '0.5px solid #b7f0c8' }}>
                    <div style={{ fontSize: '14px', fontWeight: '600', color: '#1a7f37' }}>
                      💚 Müşteriler Seviyor
                    </div>
                    <div style={{ fontSize: '11px', color: '#34c759', marginTop: '2px' }}>
                      {reviews.love?.length || 0} pozitif tema
                    </div>
                  </div>
                    ...
                    {reviews.love?.map((item, i) => (
                      <div key={i} style={{ marginBottom: '10px', paddingBottom: '10px', borderBottom: i < reviews.love.length - 1 ? '0.5px solid #f5f5f7' : 'none' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '4px' }}>
                          ...
                          <div style={{ fontSize: '11px', fontWeight: '600', color: '#34c759', background: '#e8f9ee', padding: '1px 8px', borderRadius: '10px' }}>
                            {item.count} yorum
                          </div>
                        </div>
                        ...
                        <div style={{ fontSize: '11px', color: '#8e8e93', fontStyle: 'italic' }}>
                          "{item.example}"
                        </div>
                      </div>
                    ))}
                ...
                <div style={{ background: 'white', borderRadius: '12px', border: '0.5px solid #e5e5ea', overflow: 'hidden' }}>
                  <div style={{ padding: '14px 16px 10px', background: '#fff1f0', borderBottom: '0.5px solid #ffd0ce' }}>
                    <div style={{ fontSize: '14px', fontWeight: '600', color: '#c00' }}>
                      ❤️ Müşteriler Şikayet Ediyor
                    </div>
                    <div style={{ fontSize: '11px', color: '#ff3b30', marginTop: '2px' }}>
                      {reviews.hate?.length || 0} negatif tema
                    </div>
                  </div>
                  <div style={{ padding: '12px 16px' }}>
                    {reviews.hate?.map((item, i) => (
                      <div key={i} style={{ marginBottom: '10px', paddingBottom: '10px', borderBottom: i < reviews.hate.length - 1 ? '0.5px solid #f5f5f7' : 'none' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '4px' }}>
                          <div style={{ fontSize: '13px', fontWeight: '500', color: '#1d1d1f' }}>{item.word}</div>
                          <div style={{ fontSize: '11px', fontWeight: '600', color: '#ff3b30', background: '#fff1f0', padding: '1px 8px', borderRadius: '10px' }}>
                            {item.count} yorum
                          </div>
                        </div>
                        <div style={{ height: '3px', background: '#f0f0f5', borderRadius: '2px', marginBottom: '4px' }}>
              ...
              <div style={{ background: 'linear-gradient(135deg, #0071e3, #34aadc)', borderRadius: '12px', padding: '16px 20px', marginTop: '12px', color: 'white' }}>
                <div style={{ fontSize: '13px', fontWeight: '600', marginBottom: '6px' }}>
                  💡 Ürün Geliştirme Fırsatı
                </div>
                <div style={{ fontSize: '12px', opacity: 0.9, lineHeight: '1.6' }}>
                  Müşterilerin en çok şikayet ettiği <strong>"{reviews.hate?.[0]?.word}"</strong> sorununu çözen bir ürün geliştir. 
                  Mevcut rakiplerin {reviews.hate?.[0]?.count}+ yorumu bu konudan şikayet ediyor — bu bir boşluk!
                </div>
              </div>
            </div>
          ) : (
