  ...
  const fetchQuickPicks = async () => {
    ...
    try {
      ...
      setQuickPicks([
        ...
        { asin: 'B07WDMFGDB', title: "Bambu Kesme Tahtası 3'lü Set", price: 29.99, bestseller_rank: 890, reviews_count: 421, rating: 4.5, niche_score: 78, score_color: '#ff9f0a', badge: '📈 BSR Düşük', badge_bg: '#e8f9ee', badge_color: '#1a7f37', est_revenue: '$31K/ay', fba: 'FBA' },
        { asin: 'B07YHQGNMR', title: 'Resistance Bands Set 5 Seviye', price: 18.99, bestseller_rank: 2450, reviews_count: 3892, rating: 4.7, niche_score: 74, score_color: '#af52de', badge: '🌍 Global', badge_bg: '#f3e8ff', badge_color: '#7c3aed', est_revenue: '$15K/ay', fba: 'FBM' },
      ])
    } finally {
  ...
  return (
    <div style={{ fontFamily: "'Inter', sans-serif", maxWidth: '1100px' }}>
      ...
      <div style={{ marginBottom: '18px' }}>
        ...
        <div style={{ fontSize: '13px', color: '#8e8e93', marginTop: '3px' }}>Bugün {filteredPicks.length} yeni fırsat hazır — araştırmaya başla.</div>
      </div>

      {/* Metrikler */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '10px', marginBottom: '16px' }}>
        <StatCard label="Toplam Arama" value="248" sub="+12 bu ay" subColor="#34c759" iconBg="#e8f0fe"
        ...
        <StatCard label="Niş >70 Fırsat" value="34" sub="+5 yeni bugün" subColor="#34c759" iconBg="#e8f9ee"
        ...
        <StatCard label="Alibaba Tedarikçi" value="12" sub="Analiz edildi" iconBg="#fff4e0"
        ...
        <StatCard label="Ort. Kar Marjı" value="%64" sub="−2% geçen ay" subColor="#ff3b30" iconBg="#f3e8ff"
      ...
      <div style={{ background: 'white', borderRadius: '11px', border: '0.5px solid #e5e5ea', marginBottom: '12px' }}>
        <div style={{ padding: '13px 16px 10px', borderBottom: '0.5px solid #f5f5f7', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <div style={{ fontSize: '13px', fontWeight: '600', color: '#1d1d1f' }}>⚡ Quick Picks — Günün Fırsatları</div>
        ...
        {picksLoading ? (
          <div style={{ padding: '40px', textAlign: 'center' }}>
            ...
            <div style={{ fontSize: '12px', color: '#8e8e93' }}>Fırsatlar yükleniyor...</div>
          </div>
        ) : (
            ...
            {filteredPicks.slice(0, 8).map((pick, i) => (
              ...
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '8px' }}>
                  <div style={{ fontSize: '10px', padding: '2px 8px', borderRadius: '4px', background: pick.badge_bg || '#f5f5f7', color: pick.badge_color || '#8e8e93', fontWeight: '500' }}>
                    {pick.badge || '🔥 Trend'}
                  </div>
                  <div style={{ fontSize: '14px', fontWeight: '600', color: pick.score_color || '#34c759' }}>
                    {pick.niche_score}
                  </div>
                </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr', gap: '12px', marginBottom: '12px' }}>
        ...
        <div style={{ background: 'white', borderRadius: '11px', border: '0.5px solid #e5e5ea' }}>
          ...
          <div style={{ padding: '8px 16px' }}>
            {activities.map((a, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: '10px', padding: '8px 0', borderBottom: i < activities.length - 1 ? '0.5px solid #f5f5f7' : 'none' }}>
