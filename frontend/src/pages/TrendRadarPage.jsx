...
function TrendChart({ datasets }) {
  ...
  const xScale = scaleLinear().domain([0, 11]).range([PAD.left, W - PAD.right])
  const yScale = scaleLinear().domain([0, maxVal]).range([PAD.top + chartH, PAD.top])
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', height: 'auto' }}>
      ...
      {yTicks.map(t => (
        <g key={t}>
          <line x1={PAD.left} y1={yScale(t)} x2={W - PAD.right} y2={yScale(t)} stroke="#e5e5ea" strokeWidth="0.5" />
      ...
      {datasets.map((d, idx) => (
        d.monthly?.length > 0 && (
          <path key={`area-${idx}`} d={areaFn(d.monthly)}
            fill={d.color} fillOpacity={datasets.length === 1 ? 0.12 : 0.07} />
        )
      ))}
      ...
      {datasets.map((d, idx) => (
        d.monthly?.length > 0 && (
          <path key={`line-${idx}`} d={lineFn(d.monthly)}
            fill="none" stroke={d.color} strokeWidth="2.5"
            strokeLinecap="round" strokeLinejoin="round" />
        )
      ))}

      {/* Dots — single dataset only */}
      {datasets.length === 1 && datasets[0].monthly?.map((v, i) => (
        <circle key={i} cx={xScale(i)} cy={yScale(v)} r="3.5"
          fill={datasets[0].color} stroke="white" strokeWidth="2" />
      ))}

      {/* X axis labels */}
      {MONTHS.map((m, i) => (
        <text key={m} x={xScale(i)} y={H - 4} textAnchor="middle" fontSize="9" fill="#8e8e93">{m}</text>
      ))}
    </svg>
  )
}
